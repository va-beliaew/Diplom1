...
std::string all::text() {
	std::string text;
	text = "\nГонка для наземного и воздушного транспорта. Расстояние: " + std::to_string(s) + "\n";
	return  text;
}
...
std::string* all::start() {
	...
	for (int i = 0; i < 7; ++i) {
		if (allR[i].on == 1) {
			text1[i] = allR[i].name + "." + " " + "Время: " + std::to_string(allR[i].result) + "\n";
		}
		else {
			continue;
		}
